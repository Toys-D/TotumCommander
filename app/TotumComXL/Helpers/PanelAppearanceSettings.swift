...
enum PanelAppearanceSettings {
    ...
    // MARK: - Линии сетки (подробный и краткий режимы)

    /// Два выключателя — вертикальные между столбцами и горизонтальные между строками —
    /// отдельно от цвета, чтобы попробовать линии и вернуть их не стоило оттенка.
    static let gridLinesVerticalKey = "gridLinesVertical"
    static let gridLinesHorizontalKey = "gridLinesHorizontal"
    /// Цвет линий на каждую тему: тот, что виден на тёмной панели, на светлой пропадает.
    static let gridLineColorHexLightKey = "gridLineColorHexLight"
    static let gridLineColorHexDarkKey = "gridLineColorHexDark"
    /// Прозрачность линий, 0…1, одна на обе темы: у чёрной на светлом и белой на тёмном
    /// одна и та же доля читается одинаково.
    static let gridLinesOpacityKey = "gridLinesOpacity"
    static let defaultGridLinesOpacity: Double = 0.14

    static func gridLineColorKey(dark: Bool) -> String {
        dark ? gridLineColorHexDarkKey : gridLineColorHexLightKey
    }

    /// Цвет до выбора: белый на тёмной, чёрный на светлой — прозрачность делает его тихим.
    static func defaultGridLineHex(dark: Bool) -> String {
        dark ? "#FFFFFF" : "#000000"
    }

    static func resolvedGridLinesOpacity(_ defaults: UserDefaults = .standard) -> CGFloat {
        let raw = defaults.object(forKey: gridLinesOpacityKey) as? Double ?? defaultGridLinesOpacity
        return CGFloat(max(0, min(1, raw)))
    }

    static func resolvedGridStyle(_ defaults: UserDefaults = .standard) -> GridLines.Style {
        GridLines.Style(vertical: defaults.bool(forKey: gridLinesVerticalKey),
                        horizontal: defaults.bool(forKey: gridLinesHorizontalKey))
    }

    /// Цвет линии из оттенка и прозрачности: прозрачность — ползунок, а не хвост кода цвета,
    /// чтобы её было видно и можно было крутить, не выбирая цвет заново.
    static func gridLineColor(hex: String, opacity: CGFloat, dark: Bool) -> NSColor? {
        let base = optionalNSColor(from: hex) ?? optionalNSColor(from: defaultGridLineHex(dark: dark))
        return base?.withAlphaComponent(opacity)
    }

    /// Цвет линий в силе, или nil, когда линий нет: оба выключателя выключены — nil при
    /// любом оттенке; включено без выбранного оттенка — цвет по умолчанию, чтобы включение
    /// всегда было видно.
    @MainActor
    static func resolvedGridLineColor() -> NSColor? {
        let d = UserDefaults.standard
        guard resolvedGridStyle(d).isOn else { return nil }
        let dark = isDarkAppearance
        return gridLineColor(hex: d.string(forKey: gridLineColorKey(dark: dark)) ?? "",
                             opacity: resolvedGridLinesOpacity(d), dark: dark)
    }
    ...
    /// Тёмная ли СИСТЕМНАЯ тема — независимо от того, что приложению навязали режимом.
    static var systemIsDark: Bool {
        let style = UserDefaults.standard.persistentDomain(forName: UserDefaults.globalDomain)?["AppleInterfaceStyle"]
        return (style as? String)?.lowercased() == "dark"
    }

    /// Режим после кнопки «луна/солнце»: тема меняется на противоположную, но если она
    /// совпадает с системной — ставится «Система», а не её закреплённая копия.
    ///
    /// Раньше кнопка всегда закрепляла тему (1 или 2), и после одного нажатия программа
    /// переставала ходить за системой насовсем: у человека стояло «Тёмная», а он ждал
    /// автопереключения вслед за системой и считал это поломкой.
    static func themeModeAfterToggle(currentIsDark: Bool, systemIsDark: Bool) -> Int {
        let wantDark = !currentIsDark
        if wantDark == systemIsDark { return 0 }
        return wantDark ? 2 : 1
    }
