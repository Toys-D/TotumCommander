...
/// Bakes a glowing cursor — the edge-feathered bar (or hand-drawn mask) over its own Gaussian
/// halo — into a bitmap ONCE and caches it. Redrawing/moving the cursor then just blits the
...
enum FeatheredCursor {
    ...
    private static func bake(barSize: NSSize, color: NSColor, blur: CGFloat, corner cornerRadius: CGFloat,
        ...
        //
        //    Сигма — в ПИКСЕЛЯХ битмапа, а ползунок — в пунктах. На экране Retina битмап
        //    вдвое плотнее, и без пересчёта размытие выходило вдвое слабее заданного:
        //    замерено 2 px/pt, при «12» мягкий край гас уже в 10 пт от строки — и выглядел
        //    обрезанным по ней.
        let scale = Double(cg.width) / Double(canvas.width)
        let feathered = ci.clampedToExtent()
            .applyingGaussianBlur(sigma: Double(blur) * scale)
            .cropped(to: ci.extent)

        // 3. Render (BAKE) it to a real bitmap once — the GPU never re-runs the blur.
        guard let baked = ciContext.createCGImage(feathered, from: ci.extent) else { return stroked(sharp) }
        let soft = NSImage(cgImage: baked, size: canvas)

        // 4. Размытое тело — и его же свет под ним. Края тела размыты полностью и выходят
        //    за строку; ореол снизу добавляет свечение, а середина остаётся плотной. Одно
        //    размытие само по себе тускнило курсор: у тонкой полосы даже середина не
        //    добирала до полной плотности.
        let glowing = NSImage(size: canvas, flipped: false) { rect in
            soft.draw(in: rect)   // ореол
            soft.draw(in: rect)   // размытое тело поверх него
            return true
        }
        return stroked(glowing)
    }
}
...
enum CursorIconZoom {
    ...
    /// `animated` — дорасти плавно (CursorLift), а не встать в размер сразу.
    static func apply(to imageView: NSImageView?, scale: CGFloat, animated: Bool = false) {
        guard let imageView else { return }
        imageView.wantsLayer = true
        if animated, imageView.bounds.width >= 1, let layer = imageView.layer {
            CursorLift.animateTransform(of: layer, to: transform(for: scale, in: layer))
            return
        }
    ...
    private static func applyNow(_ imageView: NSImageView?, _ scale: CGFloat) {
        ...
        layer.transform = transform(for: scale, in: layer)
    }
    ...
    static func transform(for scale: CGFloat, in layer: CALayer) -> CATransform3D {
        let w = layer.bounds.width, h = layer.bounds.height
        guard scale != 1, w > 0, h > 0 else { return CATransform3DIdentity }
