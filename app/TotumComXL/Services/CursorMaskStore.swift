...
/// Живое состояние переключателя «Свой курсор (маска)» для SwiftUI.
///
/// Ключ маски — с точками, и @AppStorage его не видит: переключатель читал настройку
/// напрямую и показывал то, что было при первой отрисовке. Включаешь — он тут же
/// отскакивал в «выключено», хотя маска уже применилась; редактор при сохранении включает
/// маску сам — и об этом переключатель тоже не узнавал. Здесь состояние идёт за тем же
/// уведомлением, по которому перерисовываются панели, — и показывает то, что рисуется.
@MainActor
final class CursorMaskSwitch: ObservableObject {
    @Published private(set) var isOn: Bool
    private var observer: NSObjectProtocol?

    init() {
        isOn = CursorMaskStore.isEnabled
    ...
    /// Перечитать хранилище — после уведомления или при показе страницы.
    func refresh() {
        let now = CursorMaskStore.isEnabled
        if now != isOn { isOn = now }
    }

    func set(_ on: Bool) {
        CursorMaskStore.setEnabled(on)
        refresh()
    }
}
