...
struct SettingsColorsView: View {
    ...
    @AppStorage(CursorLift.enabledKey) private var smoothAnimation: Bool = true
    @AppStorage(PanelAppearanceSettings.alternateRowsEnabledKey) private var alternateRows: Bool = false
    @AppStorage(PanelAppearanceSettings.gridLinesVerticalKey) private var gridVertical: Bool = false
    @AppStorage(PanelAppearanceSettings.gridLinesHorizontalKey) private var gridHorizontal: Bool = false
    @AppStorage(PanelAppearanceSettings.gridLinesOpacityKey) private var gridLinesOpacity: Double = PanelAppearanceSettings.defaultGridLinesOpacity
    ...
    /// Переключатель маски: живой, а не снимок настройки при первой отрисовке.
    @StateObject private var maskSwitch = CursorMaskSwitch()
    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                if gridVertical || gridHorizontal {
                    colorRow(gridLineTitle, hex: gridLineBinding,
                             allowsReset: true,
                             fallback: PanelAppearanceSettings.swiftUIColor(
                                from: PanelAppearanceSettings.defaultGridLineHex(dark: isDark),
                                fallback: .gray))
            ...
            Section(L("settings.performance.effects")) {
                ...
                HStack {
                    Toggle(L("design.cursor.customMask"), isOn: Binding(
                        get: { maskSwitch.isOn },
                        set: { maskSwitch.set($0) }))   // tells the panels; a raw write cannot
                        .onAppear { maskSwitch.refresh() }
                    Spacer()
    ...
    private var gridLineBinding: Binding<String> {
        let key = PanelAppearanceSettings.gridLineColorKey(dark: isDark)
        return Binding(
            get: { UserDefaults.standard.string(forKey: key) ?? "" },
            set: {
                if $0.isEmpty { UserDefaults.standard.removeObject(forKey: key) }
    ...
    private var gridLineTitle: String {
        L("design.gridLines.color") + " — "
