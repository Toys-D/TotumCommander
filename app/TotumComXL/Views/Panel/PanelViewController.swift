...
final class PanelNSTableView: NSTableView {
    ...
    /// Линии сетки; цвет nil — линий нет. Рисуются здесь же, в фоне, после полос и свечения
    /// курсора: поверх них, но под текстом строк. Геометрия — в GridLines.
    var gridLineColor: NSColor? { didSet { needsDisplay = true } }
    var gridStyle = GridLines.Style() { didSet { needsDisplay = true } }

    override func drawBackground(inClipRect clipRect: NSRect) {
        super.drawBackground(inClipRect: clipRect)
        defer { drawGridLines(in: clipRect) }
    ...
    private func drawGridLines(in clipRect: NSRect) {
        guard let gridLineColor, gridStyle.isOn else { return }
        let columns = (0..<numberOfColumns).map {
            (id: tableColumns[$0].identifier.rawValue, rect: rect(ofColumn: $0))
        }
        // Только строки в видимой части: их и рисуем, остальные за кадром.
        let visible = rows(in: clipRect)
        let rowRects = visible.length > 0
            ? (visible.location..<(visible.location + visible.length)).map { rect(ofRow: $0) }
            : []
        gridLineColor.setFill()
        for line in GridLines.table(columns: columns, rows: rowRects, bounds: clipRect,
                                    style: gridStyle) where line.intersects(clipRect) {
            line.fill()
        }
    }
...
final class PanelViewController: NSViewController,
    ...
    private static let appearanceKeys: [String] = [
        ...
        PanelAppearanceSettings.gridLinesVerticalKey,
        PanelAppearanceSettings.gridLinesHorizontalKey,
        PanelAppearanceSettings.gridLineColorHexLightKey,
        PanelAppearanceSettings.gridLineColorHexDarkKey,
        PanelAppearanceSettings.gridLinesOpacityKey,
    ...
    private func reloadAppearanceFromDefaults() {
        ...
        tableView?.gridLineColor = PanelAppearanceSettings.resolvedGridLineColor()
        tableView?.gridStyle = PanelAppearanceSettings.resolvedGridStyle()
    ...
    /// Список есть, а рядов нет — таблице ещё не дали места; попробовать на следующем такте,
    /// но не бесконечно.
    private var launchEntranceRetries = 0
    private func requestLaunchEntrance() {
        // Только когда таблица и есть список: в кратком режиме и в миниатюрах она живёт
        // скрытой, и её выезд не только невидим — он забирал показ у настоящего списка.
        guard viewModel.viewMode == .detailed else { return }
        let scheduled = LaunchEntrance.request(
            key: ObjectIdentifier(viewModel), enabled: CursorLift.isEnabled,
            views: { [weak self] in self.map { LaunchEntrance.tableRows($0.tableView) } ?? [] },
            width: { [weak self] in self?.tableView.bounds.width ?? 0 })
        if !scheduled, !viewModel.items.isEmpty, launchEntranceRetries < 5,
           LaunchEntrance.isPending(ObjectIdentifier(viewModel), enabled: CursorLift.isEnabled) {
            launchEntranceRetries += 1
            DispatchQueue.main.async { [weak self] in self?.requestLaunchEntrance() }
        }
    }

    /// Перезагрузить таблицу сразу — откладывать нельзя: список уже другой, а таблица со
    /// старыми рядами путается (пустые строки, пропавшие ячейки). Выезд при запуске
    /// подхватывает новые ряды сам.
    private func reloadTableAfterEntrance() {
        tableView.reloadData()
        requestLaunchEntrance()
    }
    ...
    private func bindViewModel() {
        ...
        viewModel.$tagsByPath
            .receive(on: RunLoop.main)
            .sink { [weak self] _ in self?.reloadTableAfterEntrance() }
            .store(in: &cancellables)
    ...
    private func refreshVisibleRowStates(cursorBandOnly: Bool = false) {
        ...
        for row in start..<end {
            ...
            // Тот же ряд, другое расстояние — курсор сдвинулся: рост плавный, если так
            // настроено. Ячейка под новый файл строится в другом месте и встаёт сразу.
            let previousDistance = rowView.cursorDistance
            rowView.cursorDistance = distance
            let animated = CursorLift.shouldAnimate(sameItem: previousDistance != distance,
                                                    laidOut: true, enabled: CursorLift.isEnabled)
            ...
            if iconColForZoom >= 0 {
                CursorIconZoom.apply(to: (rowView.view(atColumn: iconColForZoom) as? NSTableCellView)?.imageView,
                                     scale: CursorIconZoom.scale(atDistance: distance), animated: animated)
            }
            ...
            if nameColIdx >= 0,
               ...
               let marquee = cellView.subviews.compactMap({ $0 as? MarqueeTextField }).first {
                ...
                if animated {
                    CursorLift.animateTextGrowth(
                        of: marquee,
                        ratio: CursorLift.fontRatio(
                            previousScale: PanelAppearanceSettings.cursorFontScale(atDistance: previousDistance),
                            newScale: PanelAppearanceSettings.cursorFontScale(atDistance: distance)),
                        centred: false)
                }
    ...
    func populateContextMenu(_ menu: NSMenu, for item: FileItem?) {
        ...
        // Подсказки — зелёным то, ради чего этот файл или это место: до раскраски, она берёт
        // цвет из заголовка.
        ContextMenuHints.apply(ContextMenuHints.tinted(hintSituation(for: item)), to: menu)
        menu.applyAccentStyle()
    }

    /// Обстановка для подсказок: что под курсором и где стоит панель.
    private func hintSituation(for item: FileItem?) -> ContextMenuHints.Situation {
        var situation = ContextMenuHints.Situation()
        situation.onShelf = viewModel.state.insideStack
        situation.inTrash = viewModel.state.insideTrash
        situation.clipboardHasFiles = Self.clipboardHasFiles()
        situation.selectionCount = item == nil ? 0 : selectedOrCursorItems().count
        guard let item, item.name != ".." else { return situation }
        situation.isFile = !item.isDirectory
        situation.isArchive = !viewModel.insideArchive && viewModel.isArchiveFile(item)
        situation.isSymlink = item.isSymlink && item.symlinkTarget != nil
        situation.isVault = !viewModel.insideArchive && !viewModel.insideRemote
            && VaultService.isVault(item.path)
        situation.vaultUnlocked = situation.isVault && VaultService.isUnlocked(item.path)
        // Облако и архив: файла на диске нет, спрашивать систему не о чем.
        situation.hasOpener = item.isDirectory || viewModel.insideArchive || viewModel.insideRemote
            || ContextMenuHints.hasOpener(forFileAt: item.path)
        // Инструменты есть только у настоящих файлов на диске — как и само подменю.
        if !item.isDirectory, !viewModel.insideArchive, !viewModel.insideRemote {
            let kind = fileCategory(extension: item.fileExtension)
            situation.isPDF = kind == .pdf
            situation.isImage = kind == .image
            let targets = viewModel.operationTargets
            situation.pdfCount = targets.filter { fileCategory(extension: $0.fileExtension) == .pdf }.count
            situation.imageCount = targets.filter { fileCategory(extension: $0.fileExtension) == .image }.count
        }
        return situation
    }
    ...
    private func buildFileContextMenu(for item: FileItem, into menu: NSMenu) {
        ...
        if !viewModel.insideArchive {
            ...
            if viewModel.isArchiveFile(item) {
                // Зелёным его красит ContextMenuHints — вместе с остальными подсказками.
                menu.addStyledItem(title: L("context.unpack"), symbolName: "archivebox.fill",
                                   id: "context.unpack") { [weak self] in
                    guard let self else { return }
