...
struct FileListBriefView: NSViewRepresentable {
    ...
    func updateNSView(_ scrollView: NSScrollView, context: Context) {
        ...
        {
            collectionView.reloadData()
            // Появление списка при запуске — один раз на панель; ячейки рождаются в раскладке,
            // поэтому она форсируется до того, как их прятать и выводить. Перезагрузка во
            // время выезда подхватывается: новые ячейки продолжают то же расписание.
            LaunchEntrance.request(
                key: ObjectIdentifier(viewModel), enabled: CursorLift.isEnabled,
                views: { [weak collectionView] in
                    guard let collectionView else { return [] }
                    collectionView.layoutSubtreeIfNeeded()
                    return LaunchEntrance.ordered(collectionView.visibleItems().map(\.view))
                },
                width: { [weak collectionView] in collectionView?.bounds.width ?? 0 })
    ...
    final class Coordinator: NSObject, NSCollectionViewDataSource, NSCollectionViewDelegateFlowLayout, NSMenuDelegate {
        ...
        func updateCursorGlow() {
            ...
            cv.gridLineColor = PanelAppearanceSettings.resolvedGridLineColor()
            cv.gridStyle = PanelAppearanceSettings.resolvedGridStyle()
            cv.gridItemWidth = parent.itemWidth
...
final class BriefCollectionView: NSCollectionView {
    ...
    /// Линии сетки: под каждым элементом и между колонками, той же геометрией, что у
    /// раскладки (колонка `i / rowsPerColumn`, строка `i % rowsPerColumn`). Рисуются после
    /// полос и свечения курсора, под прозрачными ячейками с текстом.
    var gridLineColor: NSColor? { didSet { needsDisplay = true } }
    var gridStyle = GridLines.Style() { didSet { needsDisplay = true } }
    var gridItemWidth: CGFloat = 0 { didSet { needsDisplay = true } }
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        if let gridLineColor, gridStyle.isOn, numberOfSections > 0 {
            gridLineColor.setFill()
            let lines = GridLines.brief(itemCount: numberOfItems(inSection: 0),
                                        rowsPerColumn: alternateRowCount,
                                        itemWidth: gridItemWidth, itemHeight: alternateRowHeight,
                                        style: gridStyle)
            for line in lines where line.intersects(dirtyRect) { line.fill() }
        }
    }
...
final class BriefItemView: NSView {
    ...
    /// Чей файл ячейка показывает: тот же — курсор сдвинулся, рост плавный; другой —
    /// ячейку переиспользовали при прокрутке, размер сразу.
    private var representedPath: String?
    private var liftAnimated = false
    private var liftRatio: CGFloat = 1
    ...
    func configure(item: FileItem,
        ...
        let previousDistance = self.cursorDistance
        self.cursorDistance = cursorDistance
        liftAnimated = CursorLift.shouldAnimate(sameItem: representedPath == item.path && previousDistance != cursorDistance,
                                                laidOut: bounds.width >= 1, enabled: CursorLift.isEnabled)
        liftRatio = CursorLift.fontRatio(
            previousScale: PanelAppearanceSettings.cursorFontScale(atDistance: previousDistance),
            newScale: PanelAppearanceSettings.cursorFontScale(atDistance: cursorDistance))
