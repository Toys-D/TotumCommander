...
enum MarkdownStyler {
    ...
    private static func paragraphStyle(for block: Block, look: Look,
        ...
        if block.inQuote {
            ...
            let bar = NSTextBlock()
            // Без ширины блок раскладывается в ноль: каждая буква на своей строке (измерено).
            bar.setValue(100, type: .percentageValueType, for: .width)
        ...
        if block.isCode, let id = block.codeIdentity {
            ...
            let fill = codeBlocks[id] ?? {
                ...
                made.setValue(100, type: .percentageValueType, for: .width)
