...
final class MainWindowController: NSWindowController, NSToolbarDelegate, PanelActionDelegate, NSMenuItemValidation {
    ...
    @objc func toggleTheme(_ sender: Any?) {
        ...
        // Противоположная тема; совпала с системной — режим «Система», чтобы программа и
        // дальше ходила за автопереключением, а не застыла в закреплённой копии.
        let newMode = PanelAppearanceSettings.themeModeAfterToggle(
            currentIsDark: isDark, systemIsDark: PanelAppearanceSettings.systemIsDark)
    ...
    private func runSync(_ plan: DirectorySyncPlan) {
        ...
        guard !work.isEmpty || !moves.isEmpty || !toTrash.isEmpty || plan.creations > 0 else { return }

        Task { @MainActor in
            var problems: [String] = []

            // Пустые папки: файлов, которые принесли бы их с собой, нет — создаются сами.
            let creations = plan.createRight.map { (plan.rightRoot as NSString).appendingPathComponent($0) }
                + plan.createLeft.map { (plan.leftRoot as NSString).appendingPathComponent($0) }
            for path in creations {
                do {
                    try operationsService.ensureDirectoryTree(at: path)
                } catch {
                    problems.append("\((path as NSString).lastPathComponent): \(error.localizedDescription)")
                }
            }
