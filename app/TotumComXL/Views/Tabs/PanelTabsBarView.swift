...
struct PanelTabsBarView: View {
    ...
    /// Конец перетаскивания — по отпущенной кнопке, а не по performDrop: тот приходит, только
    /// если отпустили ровно над вкладкой. Отпустили в промежутке, мимо полосы или над уже
    /// переехавшей вкладкой — и она оставалась в виде «цели»: серая, с толстой обводкой, навсегда.
    @StateObject private var dragEnd = TabDragEndWatcher()
...
/// Отпущена ли кнопка мыши — значит, перетаскивание кончилось, где бы ни отпустили.
enum TabDragEnd {
    static func isOver(pressedButtons: Int) -> Bool {
        pressedButtons & 1 == 0
    }
}

/// Следит за концом перетаскивания по состоянию кнопки мыши. Таймер — в общих режимах цикла:
/// пока идёт перетаскивание, цикл крутится в режиме слежения, и обычный таймер молчит.
@MainActor
final class TabDragEndWatcher: ObservableObject {
    private var timer: Timer?

    func watch(_ onEnd: @escaping @MainActor () -> Void) {
        timer?.invalidate()
        let timer = Timer(timeInterval: 0.05, repeats: true) { [weak self] timer in
            MainActor.assumeIsolated {
                guard TabDragEnd.isOver(pressedButtons: NSEvent.pressedMouseButtons) else { return }
                timer.invalidate()
                self?.timer = nil
                // На следующем такте — после performDrop, если он всё же пришёл.
