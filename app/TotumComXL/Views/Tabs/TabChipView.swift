...
struct TabChipView: View {
    ...
    private var effectiveColor: Color {
        if let hex = tab.colorHex, let color = TabColor.color(fromHex: hex) {
            return color
        }
    ...
    private func buildContextMenu() -> NSMenu {
        ...
        for preset in TabColor.presets {
            ...
            let swatch = NSImage(size: NSSize(width: 12, height: 12), flipped: false) { rect in
                (TabColor.nsColor(fromHex: preset.hex) ?? .gray).setFill()
...
struct TabColor {
    ...
    /// Один разбор на всё приложение: #RRGGBB и #RRGGBBAA (так пишет панель выбора
    /// цвета и пипетка). Свой шестизначный разбор здесь однажды сбрасывал любой
    /// нестандартный цвет вкладки на акцентный.
    static func nsColor(fromHex hex: String) -> NSColor? {
        PanelAppearanceSettings.optionalNSColor(from: hex)
    }

    static func color(fromHex hex: String) -> Color? {
        nsColor(fromHex: hex).map { Color(nsColor: $0) }
    }
}
