...
struct CompareEntry: Identifiable {
    ...
    var defaultDirection: SyncDirection {
        // A folder of its own: only where it is missing on one side does it have somewhere to go —
        // an empty one is then created there; a full one follows its files (CompareFolders).
        if isDirectory {
            switch status {
...
struct DirectorySyncPlan {
    ...
    /// Пустые папки, которые надо создать на другой стороне: файлов в них нет, и копирование
    /// файлов их не принесёт.
    var createRight: [String] = []
    var createLeft: [String] = []
    ...
    var creations: Int { createRight.count + createLeft.count }
    var total: Int { copies + moves + deletions + creations }
}

/// Папки в сравнении. Сами по себе они не копируются: файлы внутри несут их с собой, дерево
/// создаётся при копировании. Но строка папки должна говорить, что с ней будет, и управлять всем
/// содержимым разом, а пустая папка, которой нет на другой стороне, должна там появиться.
/// Раньше у папок не было ни стрелки, ни щелчка, и пустые папки не переезжали никогда.
enum CompareFolders {
    static func isInside(_ path: String, folder: String) -> Bool {
        path.hasPrefix(folder + "/")
    }

    /// Файлы внутри папки, на любой глубине.
    static func files(in folder: String, of entries: [CompareEntry]) -> [CompareEntry] {
        entries.filter { !$0.isDirectory && isInside($0.relativePath, folder: folder) }
    }

    /// Сводка по папке, собранная ОДНИМ проходом по файлам: сколько их, есть ли что везти с
    /// каждой стороны и куда они едут. Перебирать все записи для каждой строки папки было
    /// нельзя: на 25 тысячах записей окно вставало на секунды, спиннер крутился, пока главный
    /// поток считал.
    struct Stat: Equatable {
        var files = 0
        var hasLeft = false
        var hasRight = false
        /// Общее направление файлов, nil — вразнобой (или файлов нет).
        var direction: SyncDirection?
        var mixed = false
    }

    /// Все папки над путём: «1/source/a.png» → «1», «1/source».
    ...
    static func stats(files: [CompareEntry],
                      direction: (CompareEntry) -> SyncDirection) -> [String: Stat] {
        var stats: [String: Stat] = [:]
        for file in files where !file.isDirectory {
            let fileDirection = direction(file)
            for folder in ancestors(of: file.relativePath) {
                var stat = stats[folder] ?? Stat()
                stat.files += 1
                if file.leftSize != nil { stat.hasLeft = true }
                if file.rightSize != nil { stat.hasRight = true }
                if stat.files == 1 {
                    stat.direction = fileDirection
                } else if !stat.mixed, stat.direction != fileDirection {
                    stat.mixed = true
                    stat.direction = nil
                }
                stats[folder] = stat
    ...
    /// Глубина вложенности: у верхнего уровня 0, у «1/source/a.png» 2.
    static func depth(of path: String) -> Int {
        ancestors(of: path).count
    }

    /// Папка над записью, nil у верхнего уровня.
    static func parent(of path: String) -> String? {
        ancestors(of: path).last
    }

    /// Что писать в строке: одно имя, если папка над ней стоит в списке (тогда уровни, как в
    /// проводнике, говорят сами за себя), и полный путь, если папку скрыл фильтр — иначе
    /// сдвинутое имя без родителя не понять.
    static func label(for entry: CompareEntry, visibleFolders: Set<String>) -> String {
        guard let parent = parent(of: entry.relativePath), visibleFolders.contains(parent) else {
            return entry.relativePath
        }
        return entry.name
    }

    /// Папки, в которых есть хоть один файл, — считая по всем записям.
    static func nonEmptyFolders(entries: [CompareEntry]) -> Set<String> {
        var result = Set<String>()
        for entry in entries where !entry.isDirectory {
            for folder in ancestors(of: entry.relativePath) { result.insert(folder) }
        }
        return result
    }

    /// Что будет с папкой: одно направление у всех файлов внутри — оно; вразнобой — nil;
    /// без файлов — её собственное (создать на другой стороне или не трогать).
    static func direction(own: SyncDirection, stat: Stat?) -> SyncDirection? {
        guard let stat, stat.files > 0 else { return own }
        return stat.mixed ? nil : stat.direction
    }

    /// Круг щелчка по папке: копия в каждую сторону, откуда есть что везти, и «не трогать».
    /// Переезды и Корзина для папок не предлагаются — это решается по файлам.
    static func cycle(for folder: CompareEntry, stat: Stat?) -> [SyncDirection] {
        let empty = (stat?.files ?? 0) == 0
        let hasLeft = empty ? folder.status != .rightOnly : stat!.hasLeft
        let hasRight = empty ? folder.status != .leftOnly : stat!.hasRight
        var states: [SyncDirection] = []
        if hasLeft { states.append(.toRight) }
        if hasRight { states.append(.toLeft) }
        states.append(.none)
        return states
    }

    /// Следующее по кругу; смешанное (nil) начинает круг с начала.
    static func next(after current: SyncDirection?, in cycle: [SyncDirection]) -> SyncDirection {
        guard let current, let index = cycle.firstIndex(of: current) else { return cycle[0] }
        return cycle[(index + 1) % cycle.count]
    }

    /// Направление каждому файлу внутри: копия — только тем, у кого на той стороне есть
    /// источник, остальным «не трогать».
    static func fileDirections(for next: SyncDirection, files: [CompareEntry]) -> [String: SyncDirection] {
        var result: [String: SyncDirection] = [:]
        // SyncDirection.none — именно направление: голое `.none` в словаре Swift читает как
        // Optional.none и молча удаляет ключ.
        let untouched = SyncDirection.none
        for file in files {
            switch next {
            case .toRight: result[file.relativePath] = file.leftSize != nil ? .toRight : untouched
            case .toLeft:  result[file.relativePath] = file.rightSize != nil ? .toLeft : untouched
            default:       result[file.relativePath] = untouched
            }
        }
        return result
    }

    /// Пустые папки к созданию: направление ведёт туда, где папки нет, а внутри нет ни одного
    /// файла — по `nonEmpty`, собранному по ВСЕМ записям, не по отфильтрованным: папка, чьи
    /// файлы спрятала маска, не пуста.
    static func creations(folders: [CompareEntry], nonEmpty: Set<String>,
                          direction: (CompareEntry) -> SyncDirection) -> (right: [String], left: [String]) {
        var right: [String] = [], left: [String] = []
        for folder in folders where !nonEmpty.contains(folder.relativePath) {
            switch (direction(folder), folder.status) {
            case (.toRight, .leftOnly): right.append(folder.relativePath)
            case (.toLeft, .rightOnly): left.append(folder.relativePath)
            default: break
            }
        }
        return (right, left)
...
struct DirectoryCompareDialogView: View {
    ...
    /// Папки, в которых есть файлы, — один раз после сканирования.
    @State private var nonEmptyFolders: Set<String> = []
    ...
    /// Сводки по папкам для текущего списка — считаются один раз на отрисовку и раздаются
    /// строкам, а не заново для каждой.
    private var folderStats: [String: CompareFolders.Stat] {
        CompareFolders.stats(files: adjusted, direction: direction)
    }

    /// Файлы внутри папки — только по щелчку, один проход.
    private func folderFiles(_ folder: CompareEntry) -> [CompareEntry] {
        CompareFolders.files(in: folder.relativePath, of: adjusted)
    }

    /// Щелчок по папке: всё содержимое разом в следующую сторону по кругу; пустая папка —
    /// сама.
    private func advanceFolder(_ folder: CompareEntry, stat: CompareFolders.Stat?) {
        let current = CompareFolders.direction(own: direction(folder), stat: stat)
        let next = CompareFolders.next(after: current, in: CompareFolders.cycle(for: folder, stat: stat))
        let files = folderFiles(folder)
        for (path, d) in CompareFolders.fileDirections(for: next, files: files) {
            overrides[path] = d
        }
        if files.isEmpty { overrides[folder.relativePath] = next }
    }
    ...
    private var plan: DirectorySyncPlan {
        ...
        let created = CompareFolders.creations(folders: adjusted.filter(\.isDirectory),
                                               nonEmpty: nonEmptyFolders, direction: direction)
        return DirectorySyncPlan(toRight: right, toLeft: left,
                                 ...
                                 createRight: created.right, createLeft: created.left)
    }
    ...
    private var summaryBar: some View {
        HStack(spacing: 8) {
            ...
            if plan.creations > 0 {
                Label("\(plan.creations)", systemImage: "folder.badge.plus").foregroundStyle(.secondary)
                    .help(L("compare.summary.folders"))
            }
            Spacer()
            if !excluded.isEmpty {
    ...
    private var list: some View {
        ...
        let rows = visible
        let visibleFolders = Set(rows.filter(\.isDirectory).map(\.relativePath))
        return ScrollView {
            LazyVStack(spacing: 0) {
                ForEach(rows) { entry in
                    row(entry, stats: stats, visibleFolders: visibleFolders)
    ...
    /// Сдвиг уровня вложенности, как в проводнике: файл под своей папкой.
    private static let indentStep: CGFloat = 18

    /// Папки — акцентным цветом из настроек: их видно с первого взгляда, и цвет тот же, что у
    /// остального окна.
    private var folderColor: Color {
        PanelAppearanceSettings.swiftUIColor(from: accentColorHex, fallback: .purple)
    }

    private func row(_ entry: CompareEntry, stats: [String: CompareFolders.Stat],
                     visibleFolders: Set<String>) -> some View {
        let dir = direction(entry)
        let indent = CGFloat(CompareFolders.depth(of: entry.relativePath)) * Self.indentStep
        let label = CompareFolders.label(for: entry, visibleFolders: visibleFolders)
        // Папка: стрелка по её файлам, смешанное — двунаправленная стрелка приглушённо.
        let stat = entry.isDirectory ? stats[entry.relativePath] : nil
        let folderDir: SyncDirection? = entry.isDirectory
            ? CompareFolders.direction(own: dir, stat: stat) : nil
        let folderMixed = entry.isDirectory && folderDir == nil
        return HStack(spacing: 8) {
            HStack(spacing: 6) {
                if entry.isDirectory {
                    Image(systemName: "folder.fill").font(.system(size: 11)).foregroundStyle(folderColor)
                }
                Text(label)
                    .font(.system(size: 12, weight: entry.isDirectory ? .semibold : .regular))
                    .foregroundStyle(entry.isDirectory ? folderColor : Color.primary)
            ...
            HStack(spacing: 1) {
                ...
                Image(systemName: entry.isDirectory
                      ? (folderMixed ? "arrow.left.arrow.right" : (folderDir ?? SyncDirection.none).symbol)
                      : dir.symbol)
            ...
            .foregroundStyle(entry.isDirectory
                             ? (folderMixed ? Color.secondary.opacity(0.6) : (folderDir ?? SyncDirection.none).tint)
                             : dir.tint)
            .frame(width: 34)
            .help(entry.isDirectory ? L("compare.folder.hint") : L("compare.direction.hint"))
        ...
        .contentShape(Rectangle())
        .onTapGesture {
            if entry.isDirectory { advanceFolder(entry, stat: stat) } else { advance(entry) }
        }
        .contextMenu {
            if entry.isDirectory {
                Button(L("compare.menu.excludeFolder")) {
                    excluded.formUnion(folderFiles(entry).map(\.relativePath))
                    excluded.insert(entry.relativePath)
                }
            }
            if !entry.isDirectory {
    ...
    private func scan() async {
        ...
        entries = scanned
        nonEmptyFolders = CompareFolders.nonEmptyFolders(entries: scanned)
        running = false
    }
}
