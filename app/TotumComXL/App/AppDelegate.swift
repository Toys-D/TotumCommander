...
final class SettingsWindowSizeGuard: NSObject, NSWindowDelegate {
    ...
    /// Ширина, с какой открывается. Заметно шире минимума: в нагруженных разделах
    /// минимального окна не хватало, и его приходилось растягивать при каждом открытии.
    static let openingWidth: CGFloat = 820
    /// Высота — от экрана: сколько даёт видимая часть за вычетом полей сверху и снизу,
    /// чтобы в разделы влезало как можно больше. Чем выше монитор, тем выше окно.
    ...
    /// С каким размером открыть окно: тот, что человек выставил сам, иначе стандартная
    /// ширина и высота во весь экран — но не больше, чем поместится, и не меньше минимума.
    /// Запомненное меньше минимума — мусор (старая запись, до поднятия минимума): оно
    /// прижимало бы окно к минимуму вместо стандартного размера, потому не учитывается.
    static func openingSize(remembered: NSSize?, screen: NSSize) -> NSSize {
        let roomWidth = max(screen.width - 2 * screenMargin, minimum.width)
        let roomHeight = max(screen.height - 2 * screenMargin, minimum.height)
        let valid = remembered.flatMap {
            $0.width >= minimum.width && $0.height >= minimum.height ? $0 : nil
        }
        let wanted = valid ?? NSSize(width: openingWidth, height: roomHeight)
