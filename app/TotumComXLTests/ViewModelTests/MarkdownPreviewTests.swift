...
final class MarkdownPreviewTests: XCTestCase {
    ...
    /// Сколько строк занимает кусок текста после настоящей раскладки в NSTextView шириной 800.
    @MainActor
    private func lines(of needle: String, in markdown: String) throws -> Int {
        let text = try XCTUnwrap(MarkdownStyler.render(markdown))
        ...
        var count = 0
        layout.enumerateLineFragments(forGlyphRange: glyphs) { _, _, _, _, _ in count += 1 }
        return count
    }

    /// Цитата, блок кода и текст после разделителя — обычные строки, а не столбик из букв.
    /// Блок без заданной ширины раскладывался в ноль: каждая буква цитаты на своей строке.
    @MainActor
    func test_блокиРаскладываютсяВСтроку() throws {
        XCTAssertEqual(try lines(of: "Живой документ", in: "> Живой документ: здесь всё"), 1,
                       "цитата в одну строку")
        XCTAssertEqual(try lines(of: "Живой документ", in:
            "# Заголовок\n\n> Живой документ: здесь всё\n> и дальше тоже\n\nАбзац."), 1,
                       "цитата из двух строк после заголовка")
        XCTAssertEqual(try lines(of: "let a = 1", in: "```swift\nlet a = 1\nlet b = 2\n```"), 1,
                       "строка кода в одну строку")
        XCTAssertEqual(try lines(of: "Абзац после", in: "Первый.\n\n---\n\nАбзац после черты."), 1)
    }
