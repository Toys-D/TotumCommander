...
final class ChromeThemeTests: XCTestCase {
    ...
    // MARK: - Кнопка «луна/солнце»

    /// Кнопка даёт противоположную тему, но совпавшую с системной не закрепляет, а ставит
    /// «Система» — чтобы программа и дальше переключалась вслед за системой. Раньше одно
    /// нажатие закрепляло тему насовсем, и автопереключение пропадало.
    func test_кнопкаТемыВозвращаетКСистеме() {
        typealias S = PanelAppearanceSettings
        // Система тёмная, программа тоже → нажали → светлая, закреплена.
        XCTAssertEqual(S.themeModeAfterToggle(currentIsDark: true, systemIsDark: true), 1)
        // Закреплена светлая при тёмной системе → нажали → тёмная = системная → «Система».
        XCTAssertEqual(S.themeModeAfterToggle(currentIsDark: false, systemIsDark: true), 0)
        XCTAssertEqual(S.themeModeAfterToggle(currentIsDark: false, systemIsDark: false), 2)
        XCTAssertEqual(S.themeModeAfterToggle(currentIsDark: true, systemIsDark: false), 0)
    }
}
