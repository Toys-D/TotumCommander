...
final class DialogWindowHidingTests: XCTestCase {
    ...
    // MARK: - Переключатель в настройках

    /// Переключатель показывает то, что рисуется: включили — включён; включили извне —
    /// редактор при сохранении или зеркало тем — тоже видно, без перерисовки страницы.
    /// Раньше он читал настройку напрямую и после щелчка отскакивал в «выключено».
    func test_переключательМаскиЖивой() {
        let toggle = CursorMaskSwitch()
        XCTAssertFalse(toggle.isOn)
        toggle.set(true)
        XCTAssertTrue(toggle.isOn)
        XCTAssertTrue(CursorMaskStore.isEnabled, "записано в хранилище")

        CursorMaskStore.setEnabled(false)
        RunLoop.main.run(until: Date().addingTimeInterval(0.05))
        XCTAssertFalse(toggle.isOn, "узнал по уведомлению")
